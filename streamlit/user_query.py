...
def print_state2(state: str, year: int):
    ...
    if state == "all":
        if year == "all":
            ...
            results_html.append(f"Top causes of death from 1999-2005\n")
        else:
            ...
            results_html.append(f"Top causes of death for year {year}\n")
    else:
        if year == "all":
            ...
            results_html.append(f"Top causes of death in {state} from 1999-2017\n")
        else:
            ...
            results_html.append(f"Top causes of death in {state} in {year}\n")
    ...
    for index, row in enumerate(results, start=1):
        ...
        if year == "all":
            results_html.append(f"#{index:<2}: {row[0]:<20} {num_of_deaths:<6} {row[2]:5}\n")
        else:
            results_html.append(f"#{index:<2}: {row[0]:<15} {num_of_deaths:<5}\n")
            result_dict[index] = num_of_deaths

    conn.close()

    return results_html, result_dict
